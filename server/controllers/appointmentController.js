...
const { createBookingPdf } = require("./sendPdf");
...
const appointmentController = {
    ...
    Doctor.findOne({ _id : doctorId }).then((doctor) => {
        const date = doctor.dates.id(dateId);
        const shift = date.shift.id(shiftId);
        shift.doctorOutShift = true;
        console.log(shift);
        res.send("Doctor out of shift on"+shift.shift+"at"+date.date);
    }).catch(err => {
        console.log(err);
        res.status(400).json(err);
    })
